...
try {
    ...
    if (!fullName) { return res.status(400).send({status:false,msg:"Enter a validfullname"})}
    if (!logoLink){ return res.status(400).send({status:false,msg:"Enter a valid logoLink"})}
...
const getcollege = async function(req,res){
   ...
   try {
    ...
    if (Object.keys(Cname).length==0){return res.status(400).send({status:false ,msg: "enter college name"})}
